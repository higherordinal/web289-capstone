...
<div class="recipe-gallery">
        ...
        <div class="filter-controls">
            ...
            <select name="type" class="filter-select">
                ...
                <?php foreach($types as $type) { ?>
                    <option value="<?php echo h($type->id); ?>"
                            <?php if($type_id === $type->id) echo 'selected'; ?>>
            ...
            <?php if(!empty($search) || !empty($style_id) || !empty($diet_id) || !empty($type_id) || $sort !== 'newest') { ?>
    ...
    <div class="results-summary">
        ...
        if(!empty($style_id)) {
            $style = RecipeAttribute::find_one($style_id, RecipeAttribute::TYPE_STYLE);
            if($style) $filter_descriptions[] = "in " . h($style->name) . " style";
        }
        if(!empty($diet_id)) {
            $diet = RecipeAttribute::find_one($diet_id, RecipeAttribute::TYPE_DIET);
            if($diet) $filter_descriptions[] = "for " . h($diet->name) . " diet";
        }
        if(!empty($type_id)) {
            $type = RecipeAttribute::find_one($type_id, RecipeAttribute::TYPE_TYPE);
            if($type) $filter_descriptions[] = "of " . h($type->name) . " type";
        }
